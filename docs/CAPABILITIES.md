...
- App/in-room delivery as the default delivery path.
- Connector identity/PIN gate preview for Telegram, Discord, Slack, WhatsApp, and SMS/text status.
- Robo Preview as a teaser-only surface.
- Public docs and package artifact boundary checks.
...
- Health collection from a real PC or server.
- Memory persistence or private recall.
- Guardrail enforcement.
- Terminal/browser execution.
- Robotics or IoT control.

## Status Language

The shell uses labels such as `setup needed`, `configured preview`, `live QA unknown`, and `unsupported`. These labels describe the static preview state only. They do not mean that a connector, provider, scheduler, or collector is live.
